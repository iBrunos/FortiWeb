...
const QtdSitesCRP: React.FC = () => {
  ...
  const fetchFortiwebData = async () => {
    ...
    try {
      ...
      setFortiwebData(result.resultados.fortiwebs || []);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center bg-gray-900 rounded-2xl relative p-6">
      <h2 className="text-2xl font-bold text-center mt-2 mb-4 text-white">Sites via CRP</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6 w-full justify-items-center">
        {fortiwebData.map((crp, index) => {
          const processedAdoms = processAdoms(crp.adoms);
          ...
          return (
            <div
              ...
              onClick={() => {
                if (isClickable) {
                  setSelectedCard({ ...crp, adoms: processedAdoms });
                  setPopupOpen(true);
                }
              }}
            >
              <div className="pb-6 md:pb-8 mb-6 md:mb-8 text-center text-slate-100 border-b border-slate-600">
                <p className="uppercase font-semibold text-slate-300">{crp.name}</p>
